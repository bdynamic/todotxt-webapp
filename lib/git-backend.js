...
async function syncWithRemote() {
  ...
  try {
    ...
    if (remoteBranch) {
      ...
      try {
        await git.pull(gitConfig.remoteName, remoteBranch, { '--rebase': 'true' });
        log('Pull completed');
      } catch (pullErr) {
        if (pullErr.message.includes('unrelated histories')) {
          ...
          try {
            await git.pull(gitConfig.remoteName, remoteBranch, { '--allow-unrelated-histories': null });
    ...
    try {
      await git.push(gitConfig.remoteName, 'main', { '--set-upstream': null });
      log('Push completed');
    } catch (pushErr) {
      if (pushErr.message.includes('refusing to update checked out branch')) {
        ...
        await git.push(gitConfig.remoteName, 'main', { '--force': null, '--set-upstream': null });
      } else {
        throw pushErr;
      }
    }

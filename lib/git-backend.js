...
async function initializeGitRepo() {
  ...
  try {
    ...
    // Bind-mounted TODO_DATA_DIR may be owned by a different uid than the
    // container runs as (e.g. after changing the container's user) - tell
    // git this directory is trusted so it doesn't refuse to operate on it.
    await simpleGit().addConfig('safe.directory', TODO_DATA_DIR, false, 'global');
    ...
    if (!isGitRepo) {
      ...
      git = simpleGit(TODO_DATA_DIR, { unsafe: { allowUnsafeSshCommand: true } });
      await git.init();
...
async function syncWithRemote() {
  if (syncInProgress) {
    log('Sync already in progress, skipping duplicate call');
  ...
  try {
    ...
    if (!currentBranch || currentBranch === '') {
      log('No branch exists yet, creating main branch...');
      await git.checkoutLocalBranch('main');
    } else if (currentBranch !== 'main') {
      log(`Current branch is '${currentBranch}', switching to 'main'...`);
      try {
        await git.checkout('main');
      } catch (checkoutErr) {
        log('main branch does not exist, creating it...');
        await git.checkoutLocalBranch('main');
      }
    }
    ...
    try {
      ...
      if (fetchErr.message.includes('does not appear to be a git repository')) {
        log('Remote repository is empty or does not exist yet');
      } else {
        throw fetchErr;
      }
    }
    ...
    const remoteMainExists = remoteBranches.all.includes(`${gitConfig.remoteName}/main`);
    const remoteMasterExists = remoteBranches.all.includes(`${gitConfig.remoteName}/master`);
    const remoteBranch = remoteMainExists ? 'main' : (remoteMasterExists ? 'master' : null);

    if (remoteBranch) {
      log(`Remote branch detected: ${gitConfig.remoteName}/${remoteBranch}`);
      debug('Pulling from remote...');
      try {
        await git.raw(['pull', '--rebase', gitConfig.remoteName, remoteBranch]);
        log('Pull completed');
      } catch (pullErr) {
        if (pullErr.message.includes('unrelated histories')) {
          log('Unrelated histories detected (first sync). Retrying with --allow-unrelated-histories...');
          try {
            await git.raw(['pull', '--allow-unrelated-histories', gitConfig.remoteName, remoteBranch]);
            log('Pull with --allow-unrelated-histories completed');
          } catch (mergeErr) {
            error('Pull with --allow-unrelated-histories failed:', mergeErr.message);
            throw mergeErr;
          }
        } else {
      ...
      log('No remote branch found (first push to empty remote)');
    }

    debug('Pushing to remote...');
    try {
      await git.raw(['push', '--set-upstream', gitConfig.remoteName, 'main']);
      log('Push completed');
    } catch (pushErr) {
      if (pushErr.message.includes('refusing to update checked out branch')) {
        log('Remote rejected push (checked out branch). Trying with force...');
        await git.raw(['push', '--force', '--set-upstream', gitConfig.remoteName, 'main']);
      } else {
        throw pushErr;
      }
    }
...
async function resetToRemote() {
  if (!gitConfig) {
    throw new Error('Git backend not initialized');
  }
  ...
  // Wipe and re-clone rather than fetch + reset --hard: same end state (working
  // tree matches remote exactly, local changes discarded either way), but it
  // also throws away any stale ownership/permissions on files left behind by
  // a previous container run under a different uid, and picks up the
  // remote's actual default branch instead of guessing main vs master.
  log('Resetting: wiping local data directory and re-cloning from remote...');

  // TODO_DATA_DIR is typically a bind-mount point in Docker - its contents can
  // be freely removed, but the mount point itself cannot be rmdir'd from
  // inside the container. Empty it in place instead of removing/recreating it.
  if (fsSync.existsSync(TODO_DATA_DIR)) {
    for (const entry of fsSync.readdirSync(TODO_DATA_DIR)) {
      fsSync.rmSync(path.join(TODO_DATA_DIR, entry), { recursive: true, force: true });
    }
  } else {
    await ensureDirectory(TODO_DATA_DIR);
  }
  ...
  const sshCommand = `ssh -i "${SSH_KEY_PATH}" -o StrictHostKeyChecking=no`;
  const cloneGit = simpleGit({ unsafe: { allowUnsafeSshCommand: true } });
  await cloneGit.env('GIT_SSH_COMMAND', sshCommand);

  try {
    await cloneGit.clone(gitConfig.remoteUrl, TODO_DATA_DIR, ['--origin', gitConfig.remoteName]);
  } catch (cloneErr) {
    error('Clone failed during reset:', cloneErr.message);
    throw cloneErr;
  }
  log('Clone completed');

  git = simpleGit(TODO_DATA_DIR, { unsafe: { allowUnsafeSshCommand: true } });
  await git.addConfig('safe.directory', TODO_DATA_DIR, false, 'global');
  await git.addConfig('user.name', gitConfig.userName, false, 'local');
  await git.addConfig('user.email', gitConfig.userEmail, false, 'local');

  const status = await git.status();
  log(`Reset complete, on branch: ${status.current}`);

  return {
    success: true,
    message: `Re-cloned from ${gitConfig.remoteName}/${status.current}`
  };
}
...
async function updateConfig(newConfig) {
  ...
  if (git) {
    if (newConfig.userName) {
      ...
      log('Updated Git user.name:', newConfig.userName);
    }
    if (newConfig.userEmail) {
      await git.addConfig('user.email', newConfig.userEmail, false, 'local');
      log('Updated Git user.email:', newConfig.userEmail);
    }
    ...
    if (newConfig.remoteUrl) {
      ...
      if (remoteExists) {
        log(`Updating existing remote '${gitConfig.remoteName}' to: ${newConfig.remoteUrl}`);
        await git.remote(['set-url', gitConfig.remoteName, newConfig.remoteUrl]);
      } else {
        log(`Adding new remote '${gitConfig.remoteName}': ${newConfig.remoteUrl}`);
        await git.addRemote(gitConfig.remoteName, newConfig.remoteUrl);
      }
      log('Git remote configured successfully');
    }

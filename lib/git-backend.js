...
async function resetToRemote() {
  ...
  log('Starting git reset --hard to remote...');

  // Clean up stale rebase state
  const rebaseMergeDir = path.join(TODO_DATA_DIR, '.git', 'rebase-merge');
  const rebaseApplyDir = path.join(TODO_DATA_DIR, '.git', 'rebase-apply');
  for (const dir of [rebaseMergeDir, rebaseApplyDir]) {
    if (fsSync.existsSync(dir)) {
      log(`Removing stale rebase directory: ${dir}`);
  ...
  try {
    ...
    error('Fetch failed during reset:', fetchErr.message);
    throw fetchErr;
  }
  ...
  if (!remoteBranch) {
    throw new Error('No remote branch found to reset to');
  }

  log(`Resetting to ${gitConfig.remoteName}/${remoteBranch}...`);
  await git.reset(['--hard', `${gitConfig.remoteName}/${remoteBranch}`]);
  log('Git reset --hard completed successfully');

  return {
    success: true,
    message: `Reset to ${gitConfig.remoteName}/${remoteBranch}`
  };
}
